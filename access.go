...
func (a Access) CopyTo(name string, w io.Writer) error {
	if !a.isAccessible(name) {
		return os.ErrPermission
	}
	return a.F.CopyTo(name, w)
}

func (a Access) CopyFrom(name string, r io.Reader) error {
	_, err := a.F.Stat(name)
	if err == nil && !a.isAccessible(name) {
		return os.ErrPermission
	}

	return a.F.CopyFrom(name, r)
}
