...
func ClearConflicts(f fs2.FS, dir string, mon chan string) error {

	zombies, err := fs2.GetZombies(f, dir)
	if err != nil {
		return err
	}
	...
	for _, z := range zombies {
		...
		if len(c) == 1 {
			var attr fs2.Attr
			_ = fs2.GetMeta(f, path.Join(dir, z), &attr)
	...
	for n, c := range cs {
		if len(c) == 1 && len(zCrc64s[n]) > 0 {
			rn := c[0].Name()
			err := fs2.UpdateAttr(f, path.Join(dir, rn), path.Join(dir, n), func(attr fs2.Attr) fs2.Attr {
				crc64s := attr.CRC64s
				switch len(crc64s) {
				case 0:
					attr.CRC64s = zCrc64s[n]
				case 1:
			...
			if err == nil {
				for _, z := range dead[n] {
					_ = fs2.RemoveMeta(f, path.Join(dir, z))
				}
			}
		}
	}
...
func SolveConflicts(f fs2.FS, dir string, names ...string) error {
	cs, err := getCollisions(f, dir)
	if err != nil {
		return err
	}
...
func getCollisions(f fs2.FS, dir string) (map[string][]fs.FileInfo, error) {
...
func GetConflicts(f fs2.FS, dir string, recursive bool) ([]Conflict, error) {
	...
	for n, files := range m {
		...
		for _, file := range files {
			var attr fs2.Attr
			_ = fs2.GetMeta(f, path.Join(dir, file.Name()), &attr)
